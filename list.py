...
print(f'\n*************************************\nCvičení 3\n*************************************')
persons.append(('Adam', 34, 'muž'))
persons.append(('Filip', 54, 'muž'))
persons.append(('Zuzana', 44, 'žena'))
persons.append(('Tereza', 46, 'žena'))
print(persons)

women = []
women = list(filter(lambda item: item[2] == 'žena', persons))
y = 0 # ten y jsem tu vytvoril protoze, python neumi tam dat x do toho printu, nevim proste to nefunguje
for x in women:
    print(women[y][0])
    delka = len(women[y][0])
    print('-' * delka)
